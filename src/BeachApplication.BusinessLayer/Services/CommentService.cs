...
public class CommentService(IDataContext dataContext, IUserService userService, IMapper mapper) : ICommentService
{
    public async Task<Result> DeleteAsync(Guid id)
    {
        var dbComment = await dataContext.GetAsync<Entities.Comment>(id);
        if (dbComment is null)
        {
            return Result.Fail(FailureReasons.ItemNotFound, $"No comment found with id {id}");
        }

        await dataContext.DeleteAsync(dbComment);
    ...
    {
        var query = dataContext.GetData<Entities.Comment>();
    ...
    public async Task<Result<Comment>> InsertAsync(SaveCommentRequest request)
    {
        ...
        await dataContext.InsertAsync(dbComment);
        await dataContext.SaveAsync();
    ...
    public async Task<Result<Comment>> UpdateAsync(Guid id, SaveCommentRequest request)
    {
        ...
        var dbComment = await dataContext.GetData<Entities.Comment>(trackingChanges: true).FirstOrDefaultAsync(c => c.Id == id && c.UserId == userId);
